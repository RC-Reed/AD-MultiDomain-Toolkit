...
# NOTE:
# - All domain names, hostnames, URLs, and UNC paths in this script
#   are demo placeholders (contoso.local style) for public sharing.
# - Replace them with your own environment if you adapt this for real use.
...
# Primary domain to use for older password-info style lookups (kept in case you ever want it)
...
# ---- Full DC scan for true last logon across all domains ----
function Get-TrueLastLogonFullScan {
    param(
        [string]$UserId
    )
    ...
    foreach ($domain in $Global:ADDomains) {
        ...
        try {
            $dcs = Get-ADDomainController -Server $domain -Filter * -ErrorAction Stop
        }
        catch {
            $lines += "Error getting domain controllers for ${domain}: " + $_.Exception.Message
            $lines += ""
            continue
        }

        $bestTime    = $null # best lastLogon
        $bestDC      = $null
        $bestBadTime = $null # best badPasswordTime
        $bestBadDC   = $null

        foreach ($dc in $dcs) {
            $dcHost = $dc.HostName
            try {
                $u = Get-ADUser -Identity $UserId -Server $dcHost `
                    -Properties lastLogon,badPwdCount,badPasswordTime `
                    -ErrorAction Stop

                $ll  = Convert-ADFileTime $u.lastLogon
                $bpt = Convert-ADFileTime $u.badPasswordTime

                if ($ll -or $bpt) {
                    $lines += "$dcHost : lastLogon=$ll ; badPwdTime=$bpt ; badPwdCount=$($u.badPwdCount)"
                }
                else {
                    $lines += "$dcHost : <no logon / bad password data>"
                }

                # track best lastLogon
                if ($ll -and (-not $bestTime -or $ll -gt $bestTime)) {
                    $bestTime = $ll
                    $bestDC   = $dcHost
                }

                # track most recent badPasswordTime
                if ($bpt -and (-not $bestBadTime -or $bpt -gt $bestBadTime)) {
                    $bestBadTime = $bpt
                    $bestBadDC   = $dcHost
                }
            }
            catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException] {
                $lines += "$dcHost : user not found on this DC."
            }
            catch {
                $lines += "$dcHost : error - " + $_.Exception.Message
            }
        }

        if ($bestTime) {
            $lines += "Most recent logon in ${domain}: $bestTime (DC: $bestDC)"
        }
        else {
            $lines += "No logon data found in ${domain}."
        }

        if ($bestBadTime) {
            $lines += "Most recent bad password in ${domain}: $bestBadTime (DC: $bestBadDC)"
        }
        else {
            $lines += "No bad password data found in ${domain}."
        }
...
# ---- TAB 3: TRUE LAST LOGON SCAN ----
$tabLogon = New-Object System.Windows.Forms.TabPage
$tabLogon.Text = "Logon Scan"
$tabs.TabPages.Add($tabLogon)

$labelLogUser = New-Object System.Windows.Forms.Label
$labelLogUser.Text = "User ID:"
$labelLogUser.Location = New-Object System.Drawing.Point(15, 20)
$labelLogUser.AutoSize = $true
$tabLogon.Controls.Add($labelLogUser)

$textLogUser = New-Object System.Windows.Forms.TextBox
$textLogUser.Location = New-Object System.Drawing.Point(80, 18)
$textLogUser.Width = 200
$tabLogon.Controls.Add($textLogUser)

$btnLogScan = New-Object System.Windows.Forms.Button
$btnLogScan.Text = "Run Full DC Scan"
$btnLogScan.Location = New-Object System.Drawing.Point(300, 16)
$btnLogScan.Width = 150
$tabLogon.Controls.Add($btnLogScan)

$textLogOutput = New-Object System.Windows.Forms.TextBox
$textLogOutput.Location = New-Object System.Drawing.Point(15, 60)
$textLogOutput.Multiline = $true
$textLogOutput.ScrollBars = "Vertical"
$textLogOutput.ReadOnly = $true
$textLogOutput.Width = 560
$textLogOutput.Height = 320
$tabLogon.Controls.Add($textLogOutput)

# ---------------- TOOLS TAB HELPERS & HANDLERS ----------------
...
# RSA via existing desktop shortcuts / URL
...
# Remote management via existing desktop shortcut
$btnBigFix.Add_Click({
    Open-DesktopShortcut -ShortcutName 'Remote_Management.lnk'
})

# ---------------- EVENT HANDLERS (USER + LOGON) ----------------
...
# Logon full DC scan
$btnLogScan.Add_Click({
    $userId = $textLogUser.Text.Trim()
    if (-not $userId) {
        [System.Windows.Forms.MessageBox]::Show("Please enter a user ID.","Missing input") | Out-Null
        return
    }

    $textLogOutput.Lines = @("Running full DC scan for user '" + $userId + "'...", "")

    try {
        $lines = Get-TrueLastLogonFullScan -UserId $userId
        $textLogOutput.Lines = $lines
    }
    catch {
        $textLogOutput.Lines = @("Error during logon scan: " + $_.Exception.Message)
    }
})
